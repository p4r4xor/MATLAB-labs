...
if(abs(k)>w)
    B(k+N+1)=0;
      %all frequency coefficients other than [-w,w] are zero
else
    B(k+N+1)=A(k+N+1);
    %all frequency coefficients between [-w,w] are same as input
    ...
    ylabel("x(t)")
    subplot(2,1,2);
    plot(-6*pi:0.001:6*pi,y);
      %plotting the time domain output signal
    title("Lowpass Output Signal")
    xlabel("time")
    ylabel("y(t)")
