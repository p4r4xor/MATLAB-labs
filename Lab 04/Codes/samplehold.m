...
Y=zeros(size(t_new));
%interpolating X to the co ordinates t_new using rectangular pulse
Y=interp1(t,X,t_new,'previous');
subplot(2,1,1)
plot(t,X,'b');
...
title('Interpolated signal(rectangular pulse) with T_{s}=0.0001sec');
hold on
plot(t_new,Z,'k');
%original signal
hold off
%calculating meansquare error
err1 = sum((Y-Z).*(Y-Z))/length(Y)